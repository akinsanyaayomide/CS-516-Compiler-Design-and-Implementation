...
*)
let cents_of (q:int) (d:int) (n:int) (p:int): int = q*25+d*10+n*5+p*1
...
let quartz : bool * int * bool  = (false,10,false)
...
*)
let third_of_three t = (match t with 
(_,_,x) -> x)
...
let compose_pair (p:(('b -> 'c) * ('a -> 'b))) : 'a -> 'c = match p with 
 | (f,g) -> fun x ->  f(g (x)) 
...
*) 
let rec append (l1:'a list) (l2:'a list) : 'a list = match l1 with 
  | [] -> l2
  | h::tl -> h::append tl l2
...
let rev_t (l: 'a list) : 'a list =
  let rec helper accum l1 = match l1 with
  |[] -> accum
  |h::tl -> helper (h::accum) tl
...
let rec insert (x:'a) (l:'a list) : 'a list = match l with 
  |[] -> [x]
  |h::tl -> if mem x l
                  then l 
            else if x < h
...
*)
let rec union (l1:'a list) (l2:'a list) : 'a list = match l1 with
  | [] -> l2
  | h::tl -> insert h (union tl l2) 
...
let e4 : exp = Mult(Add(Const 3L, Mult(Const 0L, Var "x")), Const 0L)
let e5 : exp = Add(Var "x", Mult(Var "y",Const 2L))
let e6 : exp = Neg(Mult(Var "a",Add(Var "b",Var "c")))
(*
   Problem 4-1
...
*)
let rec vars_of (e:exp) : string list = match e with
  | Var x -> [x]
  | Const y ->[]
...
let ctxt3 : ctxt = [("x", 0L);("y", 10L);("z", 1L)]
(*
...
*)
let rec lookup (x:string) (c:ctxt) : int64 = match c with 
  | [] -> raise Not_found
  | (a,b)::tl -> if a = x
                    then b 
                    else lookup x tl
...
let rec interpret (c:ctxt) (e:exp) : int64 = match e with 
  | Var x -> lookup x c
  | Const x -> x
...
let rec optimize (e:exp) : exp = match e with 
  | Var x -> Var x
  | Const x -> Const x 
  | Add (a,b) -> let a' = optimize a in
                  let b' = optimize b in 
...
*)
let rec compile (e:exp) : program = match e with 
  | Var x -> [IPushV x]
  | Const x -> [IPushC x]
