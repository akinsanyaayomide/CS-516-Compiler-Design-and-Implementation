; generated from: hw4programs/run8.oat
target triple = "x86_64-apple-macosx10.13.0"
define i64 @program(i64 %argc, { i64, [0 x i8*] }* %argv) {
  %_argc801 = alloca i64
  %_argv802 = alloca { i64, [0 x i8*] }*
  %_803 = alloca { i64, [0 x i64] }*
  store i64 %argc, i64* %_argc801
  store { i64, [0 x i8*] }* %argv, { i64, [0 x i8*] }** %_argv802
  %_raw_array804 = call i64* @oat_alloc_array(i64 2)
  %_array805 = bitcast i64* %_raw_array804 to { i64, [0 x i64] }*
  %_808 = getelementptr { i64, [0 x i64] }, { i64, [0 x i64] }* %_array805, i32 0, i32 1, i32 1
  store i64 2, i64* %_808
  %_806 = getelementptr { i64, [0 x i64] }, { i64, [0 x i64] }* %_array805, i32 0, i32 1, i32 0
  store i64 1, i64* %_806
  store { i64, [0 x i64] }* %_array805, { i64, [0 x i64] }** %_803
  %_id_arr810 = load { i64, [0 x i64] }*, { i64, [0 x i64] }** %_803
  %_index_pointer811 = getelementptr { i64, [0 x i64] }, { i64, [0 x i64] }* %_id_arr810, i32 0, i32 1, i32 1
  %_index_value812 = load i64, i64* %_index_pointer811
  ret i64 %_index_value812
}


declare i64* @oat_alloc_array(i64)
declare { i64, [0 x i64] }* @array_of_string(i8*)
declare i8* @string_of_array({ i64, [0 x i64] }*)
declare i64 @length_of_string(i8*)
declare i8* @string_of_int(i64)
declare i8* @string_cat(i8*, i8*)
declare void @print_string(i8*)
declare void @print_int(i64)
declare void @print_bool(i1)
