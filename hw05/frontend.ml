...
module TypeCtxt = struct
  ...
  let rec lookup_field_name (st:Ast.id) (f:Ast.id) (c : t) : (Ast.ty * Int64.t) =
    let fields = try lookup st c with Not_found -> failwith "debug" in
    match index_of f fields 0 with
    | None -> failwith "no such field"
    | Some x -> List.(nth fields x).ftyp, Int64.of_int x


  let rec lookup_field_name_1 (f:Ast.id) (c:t) : (Ast.ty * Int64.t) = 
    match c with 
    |[] -> raise Not_found 
    |(a,b) :: c -> (match index_of f b 0 with 
    |None -> lookup_field_name_1 f c 
    |Some x -> List.(nth b x).ftyp, Int64.of_int x )
end
...
let oat_alloc_struct ct (id:Ast.id) : Ll.ty * operand * stream =
  let ans_id, st_id = gensym "struct", gensym "raw_struct" in
  let ans_ty,st_typ = cmp_ty ct @@ TRef (RStruct id),Ptr I64 in
  let types = TypeCtxt.lookup id ct in
  let size = Const (Int64.of_int  (List.length types * 8)) in
  ans_ty, Id ans_id, lift
  [ st_id, Call(st_typ, Gid "oat_malloc", [I64, size])
  ; ans_id, Bitcast(st_typ, Id st_id, ans_ty) ]
...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Length e ->
    ...
    let subtype = match arr_ty with
      | Ptr (Struct [_; Array (_,t)]) -> t
      | _ -> failwith "invalid array or non-array passed to length"in
    let ptr_id, ans_id = gensym "pointer", "lenght" in
    I64, (Id ans_id), arr_code >@ lift
      [ ptr_id, Gep(arr_ty, arr_op, [Const 0L; Const 0L]) ; ans_id, Load(Ptr subtype, Id ptr_id)]
  ...
  *)
  ...
  let arr_id, arr_len_id, ans_id =
    gensym "newArrPointer", gensym "newArrlenghtPointer", gensym "newArrans" in
  let c = Ctxt.add c arr_id (Ptr arr_ty, Id arr_id) in
  let c = Ctxt.add c arr_len_id (Ptr I64, Id arr_len_id) in
  let arrcode_id = lift [ arr_id, Alloca arr_ty ; "", Store (arr_ty, arr_op, Id arr_id)] in
  let arrcode_lenght = lift [ arr_len_id, Alloca I64 ; "", Store (I64, size_op, Id arr_len_id)] in
  let vdecl, guard, after, body =
    (id, no_loc @@ CInt 0L), no_loc @@ Bop(Lt, no_loc @@ Id id, no_loc @@ Id arr_len_id),
    no_loc @@ Assn(no_loc @@ Id id,no_loc @@ Bop(Add, no_loc @@ Id id, no_loc @@ CInt 1L)),
    no_loc @@ Assn(no_loc @@ Index(no_loc @@ Id arr_id, no_loc @@ Id id), e2) in
  let checkfor = no_loc @@ For([vdecl], Some guard, Some after, [body]) in
  let _, new_arr_code = cmp_stmt tc c Void checkfor in
  arr_ty, Id ans_id,size_code >@ alloc_code >@ arrcode_id >@ arrcode_lenght >@ new_arr_code >@ lift [ans_id, Load(Ptr arr_ty, Id arr_id)]
   ...
   *)
  | Ast.CStruct (id, l) ->
  let st_typ, st_op, st_code = oat_alloc_struct tc id in
  let str, expr_stream = List.fold_left (fun (f_code, stream) (f_id, f_exp) -> 
    let expr_typ, expr_op, expr_stream = cmp_exp tc c f_exp in
    ...
    let field_ptr_code = I (field_ptr_id, Gep(st_typ, st_op, [Const 0L; i64_op_of_int f_ind])) in
    (*let st_id = match (List.hd(List.rev(st_code))) with 
    |I(a,b) -> a |_ -> failwith "abcd" in *)
    (*let st_id = (match (List.hd (List.rev st_code)) with |L(a) ->a |I(a,b) -> a |G(a,b)->a |E(a,b)->a |_ -> failwith "incompatible") in *)
    let f_ty, _ = TypeCtxt.lookup_field_name id f_id tc in
    let f_ty = cmp_ty tc f_ty in
    let field_value_cast_code = I ( field_value_id, Bitcast(expr_typ, expr_op, f_ty)) in
    let str = I ("", Store (f_ty, Id field_value_id, Id field_ptr_id)) in
    [str; field_value_cast_code; field_ptr_code] @ f_code, expr_stream @ stream
  ) ([], []) l  in
  st_typ, st_op, st_code >@ expr_stream >@ str
...
and cmp_exp_lhs (tc : TypeCtxt.t) (c:Ctxt.t) (e:exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Proj (e, i) ->
  let st_typ, st_op, st_code = cmp_exp tc c e in
  (*let st_id = (match (List.hd (List.rev st_code)) with |L(a) ->a |I(a,b) -> a |G(a,b)->a |E(a,b)->a |_ -> failwith "incompatible") in *)
  (*print_endline(s_id);*)
  let f_ty, f_ind = TypeCtxt.lookup_field_name_1 i tc in
  let ret_ty = cmp_ty tc f_ty in
  let ptr_id = gensym "field_index_id" in
  let proj_code = I (ptr_id, Gep(st_typ, st_op, [Const 0L; Const f_ind])) in
  ret_ty, Id (ptr_id), st_code >@ [proj_code]
...
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
    let a, ln, lm, cmp_id = gensym "nn", gensym "n", gensym "m", gensym "cmp" in
    let expr_typ, expr_op, expr_code = cmp_exp tc c exp in
    let notnull_ctxt = Ctxt.add c id (Ptr expr_typ, Id id) in
    let notnull_code,null_code = (cmp_block tc notnull_ctxt rt notnull),(cmp_block tc c rt null) in
    c, expr_code >:: I (cmp_id, Icmp (Ne, expr_typ, expr_op, Null)) >:: T (Cbr (Id cmp_id, a, ln))
        >:: L a >:: E(id, Alloca expr_typ) >:: I("", Store (expr_typ, expr_op, Id id)) >@ (snd(notnull_code)) >:: T(Br lm) 
        >:: L ln >@ (snd(null_code)) >:: T(Br lm)  >:: L lm
...
let cmp_global_ctxt (tc : TypeCtxt.t) (c:Ctxt.t) (p:Ast.prog) : Ctxt.t =
  ...
  List.fold_left (fun c -> function
    | Ast.Gvdecl { elt={ name; init } } ->
        Ctxt.add c name (Ptr (gexpr_typ c init.elt), Gid name)
    | _ -> c) c p
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  ...
  | CStruct (id, cs) ->
    let elts_gdecls, gs = List.fold_right
        (fun (id,cst) (elts_gdecls, gs) ->
           let gd, gs' = cmp_gexp c tc cst in
           gd::elts_gdecls, gs' @ gs) cs ([], [])
    in
    let st_ty,gid,st_id = (Namedt id),(gensym "global_struct" ),(GStruct elts_gdecls) in 
    (Ptr st_ty, GGid gid), (gid, (st_ty, st_id))::gs
