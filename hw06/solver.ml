...
module Make (Fact : FACT) (Graph : DFA_GRAPH with type fact := Fact.t) =
  struct
    ...
    let rec solver workset g = 
      ...
      | true -> g
      | false -> 
        let ind = Graph.NodeS.choose workset in 
        let entry = Graph.NodeS.elements (Graph.preds g ind) in 
        let entry_var = List.map (fun p -> Graph.out g p) entry in 
        let sum = Fact.combine entry_var in 
        let outgoing = Graph.flow g ind sum in 
        let workset_gen = Graph.NodeS.remove ind workset in 
        if (Fact.compare outgoing (Graph.out g ind)) <> 0 then 
          let successor = Graph.succs g ind in 
          let workset_final = Graph.NodeS.union workset_gen successor in 
          let g' = Graph.add_fact ind outgoing g in 
          solver workset_final g'
        else 
          solver workset_gen g
      end
